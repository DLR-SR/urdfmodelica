...
model CollisionBox
  ...
  parameter Modelica.Units.SI.Length length = 1 "box length (along n_x)";
  parameter Modelica.Units.SI.Length width = 1 "box width (along n_y)";
  parameter Modelica.Units.SI.Length height = 1 "box height (along cross(n_x,n_y))";
  Modelica.Mechanics.MultiBody.Parts.FixedRotation fixedRotation[8](r = {{length/2, width/2, height/2}, {length/2, width/2, -height/2}, {length/2, -width/2, height/2}, {length/2, -width/2, -height/2}, {-length/2, width/2, height/2}, {-length/2, width/2, -height/2}, {-length/2, -width/2, height/2}, {-length/2, -width/2, -height/2}}, each animation = false)  annotation (
    Placement(transformation(origin = {-50, 0}, extent = {{-10, -10}, {10, 10}})));
  CollisionPoint collisionPoint[8](each c = c, each d = d, each n_spring = n_spring, each mue = mue, each vd = vd, each n_x = n_x, each n_y = n_y, each n_z = n_z)  annotation (
...
equation
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio = false), graphics = {
        ...
        Rectangle(
          ...
          fillPattern = FillPattern.Solid),
        Polygon(
          points = {{-100, 0}, {-60, 30}, {-20, 30}, {-60, 0}, {-100, 0}},
          lineColor = {0, 0, 0},
          fillColor = {215, 215, 215},
          fillPattern = FillPattern.Solid),
        Polygon(
          points = {{-60, 0}, {-20, 30}, {-20, -6}, {-60, -40}, {-60, 0}},
          lineColor = {0, 0, 0},
          fillColor = {215, 215, 215},
          fillPattern = FillPattern.Solid),
