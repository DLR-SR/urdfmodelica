...
data_base = 'URDFModelica_Tutorial.Records'
...
def save_package(packagename, robotname, rootpath, packagepath, modelicapackagepath):
    with open(rootpath + '/' + packagepath + '/' + robotname + '/' + packagename + '/package.mo', 'w') as f:
        ...
        f.write('  extends URDFModelica_Tutorial.Icons.' + packagename + 'PackageIcon;\n')
        ...
        with open(readpath + elemtypetype + '.mo') as readf:
            for line in readf:
                if not line.startswith('within') and not line.startswith('model') and not line.startswith('end'):
                    if 'replaceable parameter URDFModelica_Tutorial.Records.'+elemtype in line: # special case params
...
def save_robot(robot, robotname, modelname, stationary, numCollisionLinks, numNonCollisionLinks, numRotationalJoints, numTranslationalJoints, numOtherJoints, rootpath, packagepath, modelicapackagepath, ordered_elements_positions, leafs):
    with open(rootpath + '/' + packagepath + '/' + robotname + '/' + modelname + '.mo', 'w') as f:
        ...
        if stationary:
            f.write('  extends URDFModelica_Tutorial.Icons.StationaryRobotIcon;\n')
        else:
            f.write('  extends URDFModelica_Tutorial.Icons.MobileRobotIcon;\n')
        ...
        with open(readfile) as readf:
            for line in readf:
                if line.startswith('equation'):
                    ...
                    f.write('  parameter Integer numOtherJoints = ' + str(int(numOtherJoints)) + ';\n')
                elif 'URDFModelica_Tutorial.Links.CollisionPoint' in line and stationary: # leafs collision points only if not stationary
                    while ';' not in line:
                        line = next(readf)
...
def make_Geometry_Text(geometry):
    ...
    if hasattr(geometry, 'filename'):
        gt += 'geometryType = URDFModelica_Tutorial.Types.GeometryType.mesh,\n'
        gt += 'size = {1.0, 1.0, 1.0},\n'
        gt += 'length = 1.0,\n'
        gt += 'radius = 1.0,\n'
        ...
        gt += 'geometryType = URDFModelica_Tutorial.Types.GeometryType.box,\n'
        ...
        gt += 'geometryType = URDFModelica_Tutorial.Types.GeometryType.cylinder,\n'
        gt += 'size = {1.0, 1.0, 1.0},\n'
        ...
        gt += 'geometryType = URDFModelica_Tutorial.Types.GeometryType.sphere,\n'
        gt += 'size = {1.0, 1.0, 1.0},\n'
        gt += 'length = 1.0,\n'
        gt += 'radius = '  + str(geometry.radius) + ',\n'
        gt += 'filename = "defaultMeshPath",\n'
...
def make_Joint_Text(joint):
    ...
    if joint.type == 'revolute':
        ...
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.revolute,\n'
    elif joint.type == 'prismatic':
        numAxes += 1
        moveableJoints.append([joint_counter, 1])
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.prismatic,\n'
    elif joint.type == 'continuous':
        numAxes += 1
        moveableJoints.append([joint_counter, 1])
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.continuous,\n'
    elif joint.type == 'floating':
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.floating,\n'
    elif joint.type == 'planar':
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.planar,\n'
    else: # elif joint.type == 'fixed':
        jt += 'jointType = URDFModelica_Tutorial.Types.JointType.fixedJoint,\n'
...
def main(argv):
    ...
    packagepath = 'URDFModelica_Tutorial/Examples' # this is the path from root where the robot pacakge will be created or overwritten
